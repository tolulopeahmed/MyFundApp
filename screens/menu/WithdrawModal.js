...
const WithdrawModal = ({
  ...
  const [fromAccount, setFromAccount] = useState(
    defaultFromAccount.toLowerCase()
  ); // Set the default account from prop
  ...
  const getBackgroundColor = (selectedBankName) => {
    ...
    const bank = selectedBankName
      ? bankOptions.find((option) => option.name === selectedBankName)
      : null; // Check if selectedBankName is defined
  ...
  useEffect(() => {}, [fromAccount, toAccount]);
          ...
          <View style={styles.presetAmountColumn} key={rowIndex}>
            {row.map((presetAmount, index) => (
              <TouchableOpacity
                ...
                key={index}
                onPress={() => handleAmountButtonPress(presetAmount)}
              >
  ...
  const investmentAmountField = (
    <>
      <Text style={styles.modalSubText2} alignSelf="flex-start" marginTop={20}>
        Amount
      </Text>
        ...
        <View style={styles.inputContainer}>
          <View style={styles.iconContainer}>
            <MaterialIcons
              ...
              color={
                selectedBankName
                  ? getBackgroundColor(selectedBankName)
                  : "#4C28BC"
              } // Check if selectedBankName is defined
              marginLeft={2}
            />
          </View>
    ...
    try {
      ...
      if (requestedAmount > savingsBalance) {
        ...
        Alert.alert(
          ...
          "You do not have enough balance in your SAVINGS account for this withdrawal."
        );
        return;
      }
  ...
  const handleInvestmentToSavingsTransfer = async () => {
    try {
      ...
      if (requestedAmount > investmentBalance) {
        ...
        Alert.alert(
          "Insufficient Balance",
          "You do not have enough balance in your INVESTMENT account for this withdrawal."
        );
        return;
      }
  ...
  const handleWalletToSavingsTransfer = async () => {
    try {
      ...
      if (requestedAmount > walletBalance) {
        ...
        Alert.alert(
          "Insufficient Balance",
          "You do not have enough balance in your WALLET for this withdrawal."
        );
        return;
      }
  ...
  const getUserDetailsByEmail = async (correctedEmail) => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        `Could not find user with email - '${correctedEmail}' . Please check and try again.`
      );
    }
  };
                ...
                <View style={styles.modalContent}>
                  ...
                  <View>
                    <Text style={styles.modalSubText}>
                      ...
                      >
                        Withdrawal to{" "}
                        <Text style={{ color: isDarkMode ? "grey" : "grey" }}>
                          Bank Account
                        </Text>{" "}
                        attracts...{"\n"}
                      </Text>
                      <View style={styles.arrow} />
                      <Text style={{ fontFamily: "proxima", color: "brown" }}>
                        5%
                      </Text>{" "}
                      charge if from Savings {"\n"}
                      <View style={styles.arrow} />
                      <Text style={{ fontFamily: "proxima", color: "brown" }}>
                        10%
                      </Text>{" "}
                      charge if from Investment {"\n"}
                      <View style={styles.arrow} />
                      <Text style={{ fontFamily: "proxima", color: "green" }}>
                        0%
                      </Text>{" "}
                      charge if from Wallet
                      {/*           
                  ...
                  <View style={styles.inputContainer}>
                    <View style={styles.dropdown}>
                      ...
                      >
                        ...
                        />
                        <Picker.Item
                          label={`Investment (₦${Math.floor(
                            accountBalances.investment
                          ).toLocaleString()})`}
                          value="investment"
                        />
                        <Picker.Item
                          label={`Wallet (₦${Math.floor(
                    ...
                    <View style={styles.dropdown}>
                      <Picker
                        ...
                        onValueChange={(value) => {
                          setToAccount(value);
                          console.log(
                            "Value of ToAccount field when changed:",
                            value
                          ); // Moved outside the callback
                        }}
                      >
                    ...
                    <>
                      ...
                      {userDetails && (
                        <View style={styles.modalSubText3}>
                          <Text style={styles.modalSubText4}>
                  ...
                  <View style={styles.buttonsContainer}>
                    <TouchableOpacity
                      ...
                      disabled={
                        ...
                        (toAccount === "Another User" && userEmail === "")
                      } // Disable if either amount is empty or user email is not provided
                    >
                      {processing ? (
                        <>
                          <ActivityIndicator
