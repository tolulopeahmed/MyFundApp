  ...
  const handleDateOfBirthChange = (text) => {
    ...
    if (/^\d{0,4}(\-)?\d{0,2}(\-)?\d{0,2}$/.test(text)) {
      ...
      if (numericText.length >= 5) {
        ...
        setDateOfBirth(numericText.slice(0, 4) + "-" + numericText.slice(4, 6));
      } else {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        if (response.status === 200) {
          ...
          Alert.alert(
            "KYC Update Failed",
            "KYC update failed. Please try again later."
          );
        }
      } else {
        setProcessing(false);
        ...
        <View style={styles.propertyContainer}>
          ...
          <View style={styles.progressBarContainer}>
            <Text style={styles.propertyText}>
              KYC (Know Your Customer) guidelines by CBN are meant to prevent
              your account from being used, intentionally or unintentionally, by
        ...
        <View style={styles.statusContainer}>
          <Text style={styles.propertyText}>
            ...
            <Text style={styles.statusText}>
              {" "}
              {kycStatus === "pending" ? "Not yet started" : kycStatus}
            </Text>
          </Text>
        </View>
