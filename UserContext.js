...
export const UserProvider = ({ children }) => {
  ...
  const fetchUserBankRecords = async (userToken) => {
    ...
    try {
      const response = await axios.get(
        `${ipAddress}/api/bank-accounts/get-bank-accounts/`,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
          },
  ...
  const fetchAccountBalances = async () => {
    ...
    try {
      const response = await axios.get(
        `${ipAddress}/api/get-account-balances/`,
        {
          headers: {
            Authorization: `Bearer ${userInfo.token}`,
          },
    ...
    try {
      ...
      setUserCards((prevUserCards) => {
        ...
        if (updatedUserCards[userToken]) {
          updatedUserCards[userToken] = updatedUserCards[userToken].filter(
            (card) => card.id !== cardId
          );
        }
